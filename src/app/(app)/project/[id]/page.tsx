...
export async function generateMetadata({ params }: PageProps) {
  ...
  const project = await PayloadService.getProjectById(id) as Project;
...
export default async function V2ProjectDetailPage({ params }: PageProps) {
  const { id } = await params;
  ...
  const getGoogleMapsUrl = (coordinates: [number, number]) => {
                  ...
                  <div className="flex flex-wrap gap-2 items-center mb-6 sm:gap-3 sm:mb-8">
                    {project.Fase && (
                      <div className="inline-flex items-center px-3 py-1.5 sm:px-4 sm:py-2 text-xs sm:text-sm text-white rounded-full backdrop-blur-sm bg-white/20 font-brandon-medium">
                        ...
                        {project.Locatie && project.LocatieCoordinaten && (
                          <a
                            href={getGoogleMapsUrl(project.LocatieCoordinaten)}
                            target="_blank"
                ...
                {project.Projectomschrijving && (
                  <div>
                    ...
                    <div 
                      ...
                      dangerouslySetInnerHTML={{ __html: `<p>${project.Projectomschrijving}</p>` }}
                    />
                  </div>
                )}
              </div>
